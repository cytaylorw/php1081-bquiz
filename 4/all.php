...
/* 
    查詢資料表資料，使用qa()。
        ...
        - $col使用單一值時，欄位名稱為id。
*/
function find($table,$col=null,$cond=""){
...
/* 
    查詢資料表資料，回傳一筆資料。
*/
function find1($table,$col=null,$cond="",$index=0){
    return find($table,$col,$cond)[$index];
}

/* 
    切割回傳資料，分頁使用。
*/
function limit($start,$div,$table,$col=null,$cond=""){
    return array_slice(find($table,$col,$cond),$start,$div);
}

/* 
    計算回傳資料筆數。
*/
function rc($table,$col=null,$cond=""){
    return count(find($table,$col,$cond));
}
